...
function AdminActions({ sponsors, onUpdate, onEdit, busy }) {
  const items = sponsors.flatMap((sponsor) => [
    sponsor.payment_status === 'Nog niet betaald' && { id: `${sponsor.id}-payment`, sponsor, type: 'payment', label: 'Betaling ontbreekt', action: 'Markeer als betaald' },
...
function App() {
  ...
  async function quickUpdate(sponsor, type) {
    const data = type === 'payment' ? { payment_status: 'Contant/anders', updated_at: new Date().toISOString() } : { logo_status: 'Ontvangen', updated_at: new Date().toISOString() }
    setBusy(true)
  ...
  return <div className="app-shell">
    ...
    <main id="content">
      {session && role === 'admin' && <button className="action-launch button secondary" onClick={() => setActionScreen((current) => !current)}>{actionScreen ? '← Terug naar dashboard' : 'Actielijst openen'}{!actionScreen && <b className="nav-badge">{sponsors.filter((s) => s.payment_status === 'Nog niet betaald' || s.logo_status === 'Nog niet ontvangen').length}</b>}</button>}
