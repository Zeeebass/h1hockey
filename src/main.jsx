...
const emptySponsor = { name: '', amount: '', sourced_by: '', logo_status: STATUS[0], payment_status: 'Anders', is_betaald: false, description: '' }
...
function App() {
  ...
  async function quickUpdate(sponsor, type) {
    const data = type === 'payment' ? { is_betaald: true, updated_at: new Date().toISOString() } : { logo_status: 'Ontvangen', updated_at: new Date().toISOString() }
    setBusy(true)
    ...
    else { setMessage(`${type === 'payment' ? 'Betaling' : 'Logo'} bijgewerkt voor ${sponsor.name}.`); await loadPrivateData(); await loadPublicProgress() }
  }
  ...
  const filtered = sponsors.filter((item) => `${item.name} ${item.sourced_by} ${item.description}`.toLowerCase().includes(search.toLowerCase()) && (paymentFilter === 'Alle betaaltypes' || item.payment_status === paymentFilter))
  ...
  return <div className="app-shell">
    ...
    <main id="content">
      {session && role === 'admin' && <button className="action-launch button secondary" onClick={() => setActionScreen((current) => !current)}>{actionScreen ? '← Terug naar dashboard' : 'Actielijst openen'}{!actionScreen && <b className="nav-badge">{sponsors.filter((s) => !s.is_betaald || s.logo_status === 'Nog niet ontvangen').length}</b>}</button>}
      {session ? (role === 'admin' && actionScreen ? <AdminActions sponsors={sponsors} onUpdate={quickUpdate} onEdit={setEditing} busy={busy} /> : <>
        <section className="workbench" aria-labelledby="add-sponsor-title"><div className="workbench-intro"><p className="kicker">Sponsoractie</p><h1 id="add-sponsor-title">Sponsor toevoegen</h1><p>Leg een nieuwe toezegging direct vast.</p></div>
          <form key={editing?.id || 'new'} className="sponsor-form" onSubmit={saveSponsor}>
            ...
            <div className="field"><label htmlFor="payment">Betalingstype</label><select id="payment" name="payment_status" defaultValue={formValue.payment_status}>{PAYMENT_TYPES.map((option) => <option key={option}>{option}</option>)}</select></div>
        ...
        <section className="overview" aria-labelledby="overview-title">
          <div className="section-heading"><div><p className="kicker">Overzicht</p><h2 id="overview-title">Sponsors</h2></div>{role === 'admin' && <button className="text-button settings" onClick={updateGoal}>Doel aanpassen</button>}</div>
          <div className="toolbar">
            <label className="search-field"><span className="sr-only">Zoek sponsors</span><input name="search" autoComplete="off" placeholder="Zoek sponsor of speler…" value={search} onChange={(event) => setSearch(event.target.value)} /></label>
          ...
          <div className="table-wrap"><table><thead><tr><th>Sponsor</th><th>Bedrag</th><th>Speler</th><th>Betalingstype</th>{role === 'admin' && <th>Betaald</th>}<th>Logo</th><th><span className="sr-only">Acties</span></th></tr></thead><tbody>
            {filtered.map((item) => <tr key={item.id}>
              ...
              <td>{item.sourced_by}</td>
              <td><span className="status">{item.payment_status}</span></td>
              {role === 'admin' && <td className="paid-cell"><input type="checkbox" checked={Boolean(item.is_betaald)} disabled={busy} onChange={(event) => updatePaid(item, event.target.checked)} aria-label={`${item.name}: betaald`} /></td>}
              <td><span className={`status ${item.logo_status === 'Ontvangen' ? 'received' : 'pending'}`}>{item.logo_status}</span></td>
