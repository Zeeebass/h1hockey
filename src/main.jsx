...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="content">
      {session && role === 'admin' && <nav className="admin-menu" aria-label="Beheermenu"><button className={adminScreen === 'dashboard' ? 'active' : ''} onClick={() => setAdminScreen('dashboard')}>Dashboard</button><button className={adminScreen === 'actions' ? 'active' : ''} onClick={() => setAdminScreen('actions')}>Actielijst <b className="nav-badge">{sponsors.filter((s) => !s.is_betaald || s.logo_status === 'Nog niet ontvangen').length}</b></button><button className={adminScreen === 'friends' ? 'active' : ''} onClick={() => setAdminScreen('friends')}>VriendjesH1</button><button onClick={updateGoal}>Doel aanpassen</button></nav>}
