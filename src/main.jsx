...
const STATUS = ['Nog niet ontvangen', 'Ontvangen', 'Geplaatst']
...
function App() {
  ...
  async function login(kind) {
    ...
    if (error) setMessage(`Supabase login mislukt: ${error.message} (gebruikt account: ${email})`)
    else setMessage('')
  }
  ...
  return <div className="app-shell"><header className="topbar"><div className="brand-mark">H1</div><div><p className="eyebrow">Sponsoractie · seizoen 2025/26</p><h1>Houten Heren 1</h1></div><div className="topbar-actions">{session ? <button className="button ghost" onClick={logout}>Uitloggen</button> : null}</div></header><main>
    <section className="hero"><div className="hero-copy"><p className="eyebrow accent">Samen naar de stip</p><h2>Elke sponsor telt.<br /><em>Elke meter ook.</em></h2><p className="hero-text">Volg de gezamenlijke sponsorvoortgang van Houten Heren 1.</p></div><div className="goal-card"><div className="goal-card-top"><span>Tot nu toe</span><span>{percentage}%</span></div><strong>{euro(total)}</strong><div className="progress-track"><div className="progress-fill" style={{ width: `${percentage}%` }} /></div><div className="goal-card-bottom"><span>Doel {euro(goal)}</span><span>{euro(Math.max(goal - total, 0))} te gaan</span></div></div></section>
    ...
    <section className="access-panel"><div><p className="eyebrow">Afgeschermd teamoverzicht</p><h3>{session ? `Ingelogd als ${role === 'admin' ? 'admin' : 'hockeylid'}` : 'Sponsorinformatie openen'}</h3><p>{session ? 'Wijzigingen worden direct veilig opgeslagen.' : 'De publieke pagina toont geen namen of individuele bedragen.'}</p></div>{!session && <div className="access-controls"><input type="password" placeholder="Wachtwoord" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && login('member')} /><button className="button light" disabled={busy} onClick={() => login('member')}>Hockeylid</button><button className="button orange" disabled={busy} onClick={() => login('admin')}>Admin</button></div>}</section>
  ...
  </main><footer><span>Houten Heren 1</span><span>Interne sponsoradministratie</span></footer>{editing && <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setEditing(null)}><form className="modal" onSubmit={saveSponsor}><div className="modal-heading"><div><p className="eyebrow accent">Sponsorregel</p><h2>{editing.id ? 'Sponsor bewerken' : 'Nieuwe sponsor'}</h2></div><button type="button" className="close" onClick={() => setEditing(null)}>×</button></div><label>Sponsornaam<input required name="name" defaultValue={editing.name} /></label><label>Bedrag (€)<input required type="number" min="0" step="1" name="amount" defaultValue={editing.amount} /></label><label>Binnengehaald door<input name="sourced_by" defaultValue={editing.sourced_by} /></label><div className="form-grid"><label>Logo<select name="logo_status" defaultValue={editing.logo_status}>{STATUS.map((item) => <option key={item}>{item}</option>)}</select></label><label>Betaling<select name="payment_status" defaultValue={editing.payment_status}>{PAYMENT.map((item) => <option key={item}>{item}</option>)}</select></label></div><label>Omschrijving<textarea name="description" rows="3" defaultValue={editing.description} /></label><button className="button orange full" disabled={busy}>Opslaan</button></form></div>}</div>
}
